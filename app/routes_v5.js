...
router.get('/home', function(req, res) {
	renderPage(res, {
		...
		params: {
			...
			api_file: version + '/apis/census_api.html'
		}
	})
})
...
router.get('/api', function(req, res) {
	...
	if (req.query.file) file = '/' + req.query.file + '.html'
...
router.get('/details/:id', function(req, res) {
	...
	var pageVars = {
		...
		hideBackButton: req.session.hideBackButton
	}

	if (req.session.hideBackButton) req.session.hideBackButton = false
...
router.post('/register', function(req, res) {
	...
	if (productName.length == 0) {
		...
		return
	} else if (productName == 'inuse') {
		renderPage(res, {
			path: '/register',
			params: { showError: true, inUse: true }
		})
		return
	} else if (!productName.match(/^[0-9a-zA-Z]+$/)) {
		renderPage(res, {
			path: '/register',
			params: { showError: true, notValid: true }
		})
		return
	}
